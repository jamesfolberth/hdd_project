...
for n =1: 10
    ...
    for k = 1:5
        ...
        for j=1:length(testIndex)
           ...
           index = sort ([trainIndex,testIndex(j)]);
            D = dist(index,:);
            D = D(:,index); 
           neighbors = distMatknn(D, find(index ==testIndex(j)), K);
    ...
    cossValAvg = zeros(6,6); 
    cossValSD = zeros(6,6); 

    for i =1:6
        for j =1:6
            
             cossValAvg(i,j)  = round(mean([R{1,1}(i,j),R{1,2}(i,j),R{1,3}(i,j),R{1,4}(i,j),R{1,5}(i,j),...
        ...
        cossValSD(i,j) = std([R{1,1}(i,j),R{1,2}(i,j),R{1,3}(i,j),R{1,4}(i,j),R{1,5}(i,j),...
                                    R{2,1}(i,j),R{2,2}(i,j),R{2,3}(i,j),R{2,4}(i,j),R{2,5}(i,j),...
                                    R{3,1}(i,j),R{3,2}(i,j),R{3,3}(i,j),R{3,4}(i,j),R{3,5}(i,j),...
                                    R{4,1}(i,j),R{4,2}(i,j),R{4,3}(i,j),R{4,4}(i,j),R{4,5}(i,j),...
                                    R{5,1}(i,j),R{5,2}(i,j),R{5,3}(i,j),R{5,4}(i,j),R{5,5}(i,j),...
