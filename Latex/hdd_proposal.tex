...
Based on this algorithm, if we were to take one song from our training data and used the rest to train the algorithm we have the following confusion matrix in Figure {\ref{fig:confMat}}.  The confustion matrix is structured so that the $(i,j)$ entry is the number of songs classified as genre $j$ that are actually of genre $i$.\\
